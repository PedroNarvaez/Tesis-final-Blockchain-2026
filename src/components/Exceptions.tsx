              ...
              {filteredExceptions.length === 0 ? (
                ...
                filteredExceptions.map(exc => (
                  <div
                    ...
                    aria-label={`Caso ${exc.id}: ${exc.motivo}. Prioridad ${exc.prioridad}`}
                    className={`p-4 hover:bg-slate-950/20 cursor-pointer transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none focus-visible:bg-slate-950/40 rounded-lg m-1 ${selectedCase?.id === exc.id ? 'bg-blue-600/5 border-l-4 border-l-blue-500 pl-3' : 'pl-4'}`}
                  >
