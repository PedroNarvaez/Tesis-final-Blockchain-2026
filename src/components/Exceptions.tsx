...
interface ExceptionsProps {
  ...
  onAction: (
    excId: string,
    action: "aprobado" | "investigando" | "reprocesado",
  ) => void;
}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white">
            Worklist de Excepciones
          </h1>
          <p className="text-sm text-slate-400">
            Bandeja priorizada de desviaciones contables y operacionales
        ...
        <div className="flex gap-2 bg-slate-950 p-1 border border-slate-800 rounded-lg">
          ...
          >
            Pendientes (
            {exceptions.filter((e) => e.estado === "pendiente").length})
          </button>
          <button
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
            <div className="p-4 border-b border-slate-800 bg-slate-950/20 flex justify-between items-center">
              <span className="text-xs font-bold text-white uppercase tracking-wider">
                Casos Abiertos de Riesgo
              </span>
              <span className="text-xs text-slate-400 font-semibold">
                Resolución conforme a SLA corporativo
              </span>
            </div>
            ...
            <div className="divide-y divide-slate-800/60">
              {filteredExceptions.length === 0 ? (
                <div className="text-center py-12 text-slate-500 text-sm">
                  No existen excepciones que requieran intervención en esta
                  categoría.
                </div>
              ) : (
                filteredExceptions.map((exc) => (
                  <button
                    ...
                    className={`w-full text-left p-4 hover:bg-slate-950/20 cursor-pointer transition-all flex flex-col md:flex-row md:items-center justify-between gap-4 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none focus-visible:bg-slate-950/10 ${selectedCase?.id === exc.id ? "bg-blue-600/5 border-l-4 border-l-blue-500 pl-3" : "pl-4"}`}
                  >
                    <div className="space-y-1.5">
                      ...
                      <p className="text-xs text-slate-400 line-clamp-1 max-w-[480px]">
                        {exc.detalle}
                      </p>

                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-[10px] text-slate-500">
                        <span className="flex items-center gap-1">
                          <Clock className="h-3 w-3" /> SLA: {exc.slaHoras}h
                          (Transcurrido: {exc.transcurridoHoras}h)
                        </span>
                        <span className="flex items-center gap-1">
                    ...
                    <div className="text-right shrink-0">
                      ...
                      <span className="text-sm font-bold text-rose-400 font-mono">
                        Gs. {exc.exposicion.toLocaleString()}
                      </span>
                      <span className="text-[10px] text-slate-500 block mt-0.5">
                        Confianza Score: {exc.confianza}%
                      </span>
                    </div>
                  </button>
                ))
          ...
          {selectedCase ? (
                ...
                <div className="grid grid-cols-2 gap-3 bg-slate-950 p-3 rounded-lg border border-slate-800">
                  <div>
                    ...
                    <p className="text-xs font-bold text-white font-mono mt-0.5">
                      Gs. {selectedCase.exposicion.toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <span className="text-slate-500 text-[10px] uppercase font-semibold">
                  ...
                  <div className="space-y-2 pt-2">
                    ...
                    <button
                      onClick={() => {
                        ...
                        setSelectedCase((prev) =>
                          prev ? { ...prev, estado: "investigando" } : null,
                        );
                      }}
                    ...
                    <button
                      onClick={() => {
                        ...
                        setSelectedCase((prev) =>
                          prev ? { ...prev, estado: "reprocesado" } : null,
                        );
                      }}
                  ...
                  <div className="bg-slate-950 p-4 rounded-lg border border-emerald-500/10 text-emerald-400 flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-slate-400 mt-0.5">
                        El caso fue resuelto y la firma digital corporativa ha
                        sido registrada off-chain.
                      </p>
                    </div>
                  </div>
                )}
            ...
            <div className="bg-slate-900 border border-slate-800 border-dashed rounded-xl p-8 text-center text-slate-500 text-xs flex flex-col items-center justify-center h-64">
              ...
              <p className="font-semibold text-slate-400">
                Seleccione un Caso para Resolver
              </p>
              <p className="max-w-[180px] mt-1 mx-auto text-[11px] text-slate-500">
                Visualice la evidencia de desvío de reglas y ejecute la
