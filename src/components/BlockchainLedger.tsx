...
export const BlockchainLedger: React.FC<BlockchainLedgerProps> = ({
  ...
  const [selectedBlock, setSelectedBlock] = useState<BlockchainBlock | null>(
    null,
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white">
            Libro Mayor Blockchain Permissionado
          </h1>
          <p className="text-sm text-slate-400">
            Consola de auditoría de contratos inteligentes e integridad
            criptográfica (Hyperledger Fabric)
          </p>
        </div>
        ...
        <div className="flex items-center gap-3">
          <div
            className={`px-3 py-1.5 rounded-lg border flex items-center gap-2 text-xs font-bold ${isChainValid ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400" : "bg-rose-500/10 border-rose-500/30 text-rose-400"}`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex gap-3 items-center">
          ...
          <div className="min-w-0">
            <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">
              Hash del Dataset DNIT Paraguay (Contexto Fiscal)
            </span>
            <p className="text-xs font-mono text-blue-300 truncate mt-1">
              {dnitHash}
            </p>
          </div>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl flex gap-3 items-center">
          ...
          <div className="min-w-0">
            <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">
              Contrato Inteligente de Reglas de Conciliación
            </span>
            <p className="text-xs font-semibold text-slate-200 mt-1">
              ReconciliationRulesetContractv1.0.3 · Activo
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-sm text-white uppercase tracking-wider">
                  Secuencia de Bloques Confirmados (Ledger Stream)
                </h3>
              </div>
              <span className="text-xs text-slate-400 font-semibold">
                {blocks.length} Bloques de Evidencias
              </span>
            </div>
              ...
              {blocks.map((block) => {
                ...
                return (
                  <button
                    ...
                    aria-label={`Bloque ${block.indice}: ${block.evidencias.datosResumen}`}
                    aria-selected={isSelected}
                    className={`w-full text-left p-4 rounded-lg border hover:border-slate-600 transition-all cursor-pointer flex items-center justify-between gap-4 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none focus-visible:bg-slate-950/10 ${
                      isSelected
          ...
          {selectedBlock ? (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-6 h-fit sticky top-6">
              <div className="border-b border-slate-800 pb-4">
                ...
                <h3 className="text-sm font-bold text-white mt-1">
                  Registro de Consistencia de Conciliación
                </h3>
              </div>
              ...
              <div className="space-y-4 text-xs">
                ...
                <div className="space-y-3 bg-slate-950 p-3 rounded-lg border border-slate-800">
                  <span className="text-[10px] text-slate-400 font-bold uppercase border-b border-slate-800 pb-1.5 block">
                    Evidencias Almacenadas (On-Chain)
                  </span>

                  <div>
                    <span className="text-[10px] text-slate-500 font-medium">
                      ID de Cruce Vinculado:
                    </span>
                    <p className="font-semibold text-white font-mono mt-0.5">
                      {selectedBlock.evidencias.matchId}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-slate-300 mt-0.5">
                      {selectedBlock.evidencias.tipo}
                    </p>
                  </div>

                  <div>
                    <span className="text-[10px] text-slate-500 font-medium">
                      Hash del Documento Original (Off-Chain):
                    </span>
                    <p className="font-mono text-slate-300 truncate mt-0.5">
                      {selectedBlock.evidencias.hashDocumento}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <p className="font-semibold text-slate-300 mt-0.5">
                      {selectedBlock.evidencias.datosResumen}
                    </p>
                  </div>
                </div>
                ...
                <div className="p-3 bg-blue-600/5 rounded-lg border border-blue-500/10 flex gap-2.5 items-start">
                  ...
                  <div>
                    <span className="text-[10px] text-slate-400 font-bold block uppercase">
                      Chaincode de Consenso
                    </span>
                    <p className="text-[11px] text-slate-300 font-medium mt-0.5">
                      {selectedBlock.contratoInteligente}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-slate-900 border border-slate-800 border-dashed rounded-xl p-8 text-center text-slate-500 text-xs flex flex-col items-center justify-center h-64">
              ...
              <p className="font-semibold text-slate-400">
                Examine un Bloque de Evidencia
              </p>
              <p className="max-w-[180px] mt-1 mx-auto text-[11px] text-slate-500">
                Seleccione cualquier bloque de la cadena para auditar las firmas
                criptográficas de la conciliación.
              </p>
            </div>
          )}
        </div>
